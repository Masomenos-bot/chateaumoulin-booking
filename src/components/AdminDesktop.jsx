...
const BG="#F5F3ED",FG="#000",MU="#999",BD="#e0dcd0",CW=42;
const ROSE="#E8A0B4",ROSE_BG="#F2D4DD",RS="#C06080";
const LAV="#8B9DC3",LAV_BG="#C8D2E4",LS="#5C6F94";
...
export default function App(){
  ...
  const hereNow=occDayBookings.filter(b=>b.status==="confirmed");
  ...
  const Card=({b})=>{
    ...
    return(<div onClick={()=>setModal({type:"edit",data:b})} style={{padding:"8px 10px",marginBottom:4,cursor:"pointer",borderLeft:isPre?`2px solid ${LAV}`:`2px solid ${ROSE}`,background:isPre?LAV_BG:ROSE_BG,transition:"padding-left 0.15s"}}
  ...
  return(<div style={{fontFamily:C,background:BG,minHeight:"100vh",color:FG}}>
    ...
    <div style={{padding:"16px 28px",borderBottom:"2px solid #000",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div style={{display:"flex",alignItems:"center",gap:16}}>
        <img src={EYE} alt="" style={{height:32,opacity:0.8}} />
        <div>
          <div style={{fontSize:18,fontWeight:700,letterSpacing:"0.12em",fontFamily:H}}>CHÂTEAUMOULIN</div>
          <div style={{fontSize:9,color:MU,letterSpacing:"0.2em",marginTop:2,fontFamily:C}}>BOOKING · SEASON 2026 · JUN 15 — SEP 15</div>
        </div>
      </div>
      <button onClick={()=>setModal({type:"new",data:null})} style={{padding:"8px 20px",border:"2px solid #000",background:"#000",color:BG,cursor:"pointer",fontFamily:C,fontSize:10,fontWeight:700,letterSpacing:"0.15em"}}>+ NEW BOOKING</button>
    </div>

    <div style={{padding:"12px 28px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${BD}`}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <button onClick={prev} style={{background:"none",border:"1px solid #ccc",padding:"5px 10px",cursor:"pointer",fontFamily:H,fontSize:14,color:FG}}>←</button>
        <span style={{minWidth:160,textAlign:"center",fontSize:15,fontWeight:700,letterSpacing:"0.08em",fontFamily:H}}>{MS[month].toUpperCase()} {year}</span>
        <button onClick={next} style={{background:"none",border:"1px solid #ccc",padding:"5px 10px",cursor:"pointer",fontFamily:H,fontSize:14,color:FG}}>→</button>
        <button onClick={goNow} style={{background:"none",border:"1px solid #000",padding:"5px 12px",cursor:"pointer",fontFamily:C,fontSize:9,fontWeight:700,letterSpacing:"0.1em",color:FG}}>TODAY</button>
    ...
    {selDay&&(<div style={{padding:"16px 28px 0"}}><DayPanel dk={selDay} bookings={bookings} rooms={ROOMS} onClose={()=>setSelDay(null)} onEdit={b=>setModal({type:"edit",data:b})} /></div>)}

    <div style={{display:"flex",padding:"20px 28px"}}>
      <div style={{flex:1,minWidth:0}}>
        <div ref={ref} style={{overflowX:"auto",scrollBehavior:"smooth"}}>
          <div style={{width:dim*CW+80}}>
            <div style={{display:"flex",borderBottom:"2px solid #000"}}>
              <div style={{width:80,flexShrink:0,padding:"8px 10px",fontSize:9,fontWeight:700,letterSpacing:"0.15em",fontFamily:C}}>SLOTS</div>
              <div style={{display:"flex"}}>
                {days.map(d=>{
                  ...
                  return(<div key={d} onClick={()=>inS(key)?setSelDay(selDay===key?null:key):null}
                    style={{width:CW,textAlign:"center",padding:"4px 0",fontSize:11,fontFamily:H,fontWeight:isT?800:isSel?700:400,color:off?"#d0ccc0":isT?"#000":we?"#aaa":"#888",background:isSel?"rgba(0,0,0,0.08)":isT?"rgba(0,0,0,0.04)":"transparent",cursor:off?"default":"pointer",borderBottom:isSel?"2px solid #000":"2px solid transparent",flexShrink:0}}>
                    <div style={{fontSize:8,color:off?"#d0ccc0":MU,fontFamily:C}}>{dn(key)}</div><div>{d}</div>
                  </div>);
                })}
              </div>
            </div>
            {ROOMS.map((room,ri)=>(
              <div key={room.id} style={{display:"flex",borderBottom:`1px solid ${BD}`}}>
                <div style={{width:80,flexShrink:0,padding:"10px 12px",fontSize:12,fontWeight:600,display:"flex",alignItems:"center",color:MU,fontFamily:H}}>{ri+1}</div>
                <div style={{display:"flex",alignItems:"center",minHeight:38}}>
                  {days.map(d=>{
                    ...
                    if(off)return <div key={d} style={{width:CW,height:38,flexShrink:0,background:"repeating-linear-gradient(45deg,transparent,transparent 3px,rgba(0,0,0,0.03) 3px,rgba(0,0,0,0.03) 4px)"}} />;
                    if(b){const isPre=b.status==="prebooking";
                      return(<div key={d} style={{width:CW,height:38,flexShrink:0,display:"flex",alignItems:"center",cursor:"pointer",position:"relative",background:isSel?"rgba(0,0,0,0.04)":"transparent"}} onClick={()=>setModal({type:"edit",data:b})} title={`${b.guest} (${b.roomIds.length}rm)`}>
                        <div style={{position:"absolute",left:isCI?2:0,right:0,top:5,bottom:5,background:isPre?LAV_BG:ROSE_BG,borderLeft:isCI?`2px solid ${isPre?LAV:ROSE}`:"none"}}>
                          {isCI&&<div style={{fontSize:8,fontWeight:700,padding:"2px 4px",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",color:isPre?LS:RS,letterSpacing:"0.05em",fontFamily:C}}>{b.guest.split(" ")[0].toUpperCase()}</div>}
                        </div>
                      </div>);
                    }
                    return(<div key={d} style={{width:CW,height:38,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",background:isSel?"rgba(0,0,0,0.04)":"transparent"}}
      ...
      <div style={{width:240,flexShrink:0,paddingLeft:20,borderLeft:`1px solid ${BD}`}}>
        <OccBar bookings={bookings} rooms={ROOMS} refDay={occDay} label={occLabel} />
        {hereNow.length>0&&(<div style={{marginBottom:24}}><div style={{fontSize:9,fontWeight:700,letterSpacing:"0.2em",color:MU,marginBottom:8,fontFamily:C}}>{selDay?"IN HOUSE":"HERE NOW"}</div>{hereNow.map(b=><Card key={b.id} b={b} />)}</div>)}
        {prebs.length>0&&(<div style={{marginBottom:24}}><div style={{fontSize:9,fontWeight:700,letterSpacing:"0.2em",color:MU,marginBottom:8,fontFamily:C}}>PRE-BOOKINGS ({prebs.length})</div>{prebs.map(b=><Card key={b.id} b={b} />)}</div>)}
        {upcoming.length>0&&(<div style={{marginBottom:24}}><div style={{fontSize:9,fontWeight:700,letterSpacing:"0.2em",color:MU,marginBottom:8,fontFamily:C}}>UPCOMING</div>{upcoming.map(b=><Card key={b.id} b={b} />)}</div>)}
        <div style={{marginTop:16,fontSize:9,color:MU}}>
          <div style={{fontSize:9,fontWeight:700,letterSpacing:"0.2em",marginBottom:8,fontFamily:C}}>PRICING GRID — €/NIGHT</div>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:10,fontFamily:C}}>
            <thead><tr style={{borderBottom:`1px solid ${BD}`}}><td style={{padding:"4px 6px"}}></td><td style={{padding:"4px 6px",textAlign:"center",fontWeight:700,fontFamily:H}}>T1</td><td style={{padding:"4px 6px",textAlign:"center",fontWeight:700,fontFamily:H}}>T2</td><td style={{padding:"4px 6px",textAlign:"center",fontWeight:700,fontFamily:H}}>T3</td></tr>
            <tr style={{borderBottom:`1px solid ${BD}`}}><td style={{padding:"2px 6px"}}></td><td style={{padding:"2px 6px",textAlign:"center",fontSize:8,color:"#bbb"}}>{"<"}May 15</td><td style={{padding:"2px 6px",textAlign:"center",fontSize:8,color:"#bbb"}}>{"<"}Jul 1</td><td style={{padding:"2px 6px",textAlign:"center",fontSize:8,color:"#bbb"}}>Jul 1+</td></tr></thead>
            <tbody>{[["1-2n",200,270,320],["3-5n",185,265,305],["6+n",170,255,290]].map(([l,...v],i)=>(<tr key={i} style={{borderBottom:`1px solid ${BD}`}}><td style={{padding:"4px 6px",color:MU}}>{l}</td>{v.map((x,j)=><td key={j} style={{padding:"4px 6px",textAlign:"center",color:FG}}>{x}€</td>)}</tr>))}</tbody>
          </table>
        </div>
        <div style={{marginTop:40,textAlign:"center"}}><img src={EYE} alt="" style={{height:24,opacity:0.15}} /></div>
      </div>
    </div>
